...
public class BooleanComponent extends Component{
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        mc.textRenderer.drawWithShadow(matrices,((OptionBoolean) option).isEnabled() ? TextUtil.GREEN + ((OptionBoolean) option).getName() : TextUtil.RED + ((OptionBoolean) option).getName(),hackButton.window.x + 1, hackButton.window.y + hackButton.yOffset + yOffset + 1, -1);
