...
public class Ping extends HudElement {
    ...
    public void onRender(Render2DEvent e) {
        if(nullCheck()){return;}
        e.getContext().drawTextWithShadow(mc.textRenderer, TextUtil.AQUA + "Ping:" + TextUtil.WHITE + (mc.player.networkHandler.getPlayerListEntry(mc.player.getGameProfile().getId()) == null ? 0 : mc.player.networkHandler.getPlayerListEntry(mc.player.getGameProfile().getId()).getLatency()), 0, mc.getWindow().getScaledHeight() - (mc.textRenderer.fontHeight * 2), -1);
